...
* **`train.py`**: The central engineering pipeline. Cleans text variations, routes features through parallel `ColumnTransformer` layouts, evaluates baseline architectural stability cleanly via cross-validation, and serializes the 100% data-fitted models into a local binary file.
* **`app.py`**: A lightweight, interactive live Command Line Interface (CLI). Loads the serialized model cluster, captures real-time user inputs, validates format inputs, and displays immediate star classifications wrapped with dynamic instance-level certainty scores (`predict_proba`).
